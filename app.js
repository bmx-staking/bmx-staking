...
(function () {
  ...
  function currentUser() {
    const s = getSession();
    if (!s) return null;
    return migrateUser(users().find(u => u.id === s.id) || null);
  }
  ...
  function migrateUser(user) {
    if (!user) return null;
    let changed = false;
    if (!user.accountId) { user.accountId = makeAccountId(); changed = true; }
    if (!user.referralCode) { user.referralCode = user.accountId; changed = true; }
    if (!Array.isArray(user.referrals)) { user.referrals = []; changed = true; }
    if (changed) {
      const list = users();
      const idx = list.findIndex(u => u.id === user.id);
      if (idx >= 0) { list[idx] = user; save(USERS_KEY, list); }
    }
    return user;
  }
  ...
  function createUser(name, email, password, referralCode) {
    ...
    user.referralCode = user.accountId;
    list.push(user);
    save(USERS_KEY, list);
  ...
  function ensureAdminAccount() {
    ...
    if (!admin) {
      ...
      if (!admin.accountId) admin.accountId = "BMXADMIN";
      if (!admin.referralCode) admin.referralCode = admin.accountId;
      save(USERS_KEY, list);
    }
    return admin;
  }
  ...
  function protectPages() {
    ...
    const publicPages = ["", "index.html", "login.html", "register.html", "faq.html", "support.html", "admin-login.html"];
  ...
  function personalize() {
    ...
    document.querySelectorAll("[data-account-id]").forEach(el => el.textContent = u.accountId || "—");
    document.querySelectorAll("[data-referral-link]").forEach(el => el.value = referralUrl(u.referralCode || u.accountId));
  }
