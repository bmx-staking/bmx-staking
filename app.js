...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  if(stake) stake.onsubmit=e=>{
    ...
    save(); refresh(); toast(fmt(amt)+" BMX staked in demo mode");
  };
  ...
  if(transfer) transfer.onsubmit=e=>{
    ...
    save(); refresh(); transfer.reset(); toast("Demo transfer recorded");
  };

  const register=document.querySelector("#registerForm");
  if(register) register.onsubmit=e=>{e.preventDefault();toast("Demo account created");setTimeout(()=>location.href="login.html",450);};
  const login=document.querySelector("#loginForm");
  if(login) login.onsubmit=e=>{e.preventDefault();toast("Demo sign-in successful");setTimeout(()=>location.href="dashboard.html",450);};
